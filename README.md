...
Press F near the blacksmith to buy, equip, preview, or upgrade gear. The shop is laid out as a page from the same illuminated book as the opening cinematic: a shaded vellum ground, cut-cornered steel plates, gilt hairlines and a display serif for anything that names a thing. Gear cards on the left carry item icons rendered from the real 3D gear, the appraisal plate in the middle holds stat bars and comparisons against worn gear with the action fixed to a rail at its foot, and the mannequin stands unframed in the forge light down the right-hand side. Level locks, owned copies and worn pieces are badged on the cards, and the anvil tab shows temper ranks and survival odds as gauges. The shop stocks three greatswords and two full plate sets with independent helmet, chest, gloves and boots slots. Duplicate purchases have separate identities and upgrade ranks. Equipping changes the animated player and first-person equipment; the fitting preview also works without purchasing. Research is available at the archive. Sparring in the eastern yard earns gold and XP. No debug currency or selling is exposed.
...
Press I to open the inventory outside combat. It shares the forge and archive page, lit by one low candle: what you are wearing stands unframed on the left, the bag fills the middle with icons rendered from the real gear, and the appraisal on the right compares against the piece in that slot over a fixed action rail. Inspect, filter and sort owned gear, equip or remove all five slots, rotate the equipped character preview, and view gold, diamonds and living essence. Each duplicate keeps its own upgrade rank. Equipment changes save immediately and roll back on write failure. Selling, dropping and consumables are not part of this inventory pass.
...
Press F at the researcher to open branching Character, Equipment, Trade and Scholarship trees, plus a WIP Hub tree. The archive shares the armoury's look, lit from a cold window instead of a fire: study plates with tree emblems, rank pips and inked prerequisite connectors drawn straight onto the page, an appraisal plate with per-rank bonuses, costs and timers over a fixed action rail, and a strip of research desks with live progress. The blacksmith, archive and inventory all draw their chrome from `scripts/forge_ui.gd`, which owns the page shader, the cut plates, the display serif and the shared appraisal plate; `scripts/gear_thumbnails.gd` renders the item icons once and every screen reads the same cache. The classroom and menus still use `scripts/ui_kit.gd`. Two concurrent studies expand to four; projects can be paused without losing progress. Research starts with gold costs and later adds diamonds, completes automatically with notifications, and progresses at 25% speed offline (improvable to 50%). The longest studies take 5–10 minutes of play. See `docs/RESEARCH.md` for the current prototype balance and `--research-test` for isolated checks. Character stat bonuses are recorded for the forthcoming combat system.
