...
func run(game: Node) -> void:
	...
	menu.intro.finish()
	await wait(.1)
	check(not get_tree().paused and not is_instance_valid(menu.intro),"Skipping releases the cinematic and resumes the world")
	Input.action_press("right")
	await wait(1)
	Input.action_release("right")
	await wait(.3)
	...
	check(not get_tree().paused and not game.input_blocked,"Natural cinematic completion restores control")
	await wait(.1)
	menu.save_game()
